...
if not st.session_state['login']:
    ...
    if st.button("Login"):
        if authenticate(username, password):
            ...
            st.experimental_rerun()  # Re-run the script after successful login to load the dashboard
        else:
            st.error("Invalid username or password.")
else:
    decoded_token = verify_jwt_token(st.session_state['jwt_token'])
    if decoded_token:
        st.title(":bar_chart: Broadcasting Performance Dashboard")
        #st.sidebar.image("Analytics.png", width=200)  # Add your logo here
